            ...
            <div class="col-8">
                ...
                <form>
                    ...
                    <fieldset>
                        ...
                        @endif
                        <div id="divpml">
                            <label for="pmllist" class="">Pengawas</label>
                            <input class="
                                        form-control" list="pmloption" id="pmllist" placeholder="Type to search..."
                                name="pml" autocomplete="off" onClick="this.setSelectionRange(0, this.value.length)">
                            <datalist id="pmloption">
                                {{-- <option value="1600" selected>Semua</option> --}}
                                @foreach ($pmllist as $pml)
                                <option value={{ $pml->kode }} selected>{{ $pml->nm_petugas }}
                                </option>
                                @endforeach
                            </datalist>
...
<script>
    $(".tanggal").addClass("active");
        ...
        //var pml1 = {{ json_encode($pml1) }};
        if (request.pml != null) {
            $('#pmllist').val(request.pml);
        } else {
        ...
        //    console.log(kd_kab)
           window.location.href = "downloadtabeltanggalbypml?kab="+kd_kab+"&petugas="+ petugas+
           "&nks="+ nks
        })
</script>
@endsection
